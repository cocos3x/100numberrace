...
// =======================
// HÀM ENTRY POINT CHO APPS SCRIPT
// =======================
...
var output  = LodeOnline.lines.join("\n");
  var copyText = LodeOnline.copy || "";
    var copyText25 = LodeOnline.copy25 || "";
      var copyText36 = LodeOnline.copy36 || "";
        var copyText49 = LodeOnline.copy49 || "";
               var copyText16 = LodeOnline.copy16 || "";
                   var copyText33 = LodeOnline.copy33 || "";
 ...
 <html>
  <head>
    ...
    <style>
      ...
      /* 3 nút thêm */
      #copyBtn2, #copyBtn3, #copyBtn4 ,#copyBtn5,#copyBtn6{
        position: fixed;
        right: 10px;
        background: #007bff;
        color: white;
 ...
 #copyBtn6:active {
  ...
  <body>
    ...
    <!-- NÚT GỐC -->
    <button id="copyBtn">📋 Copy 50 số</button>

    <!-- 3 NÚT COPY BỔ SUNG -->
    <button id="copyBtn2">📋 Copy 25</button>
    <button id="copyBtn3">📋 Copy 36</button>
    <button id="copyBtn4">📋 Copy 49</button>
        <button id="copyBtn5">📋 Copy 16</button>
                <button id="copyBtn6">📋 Copy 9</button>

    <h3>Triệu hồi BẠCH THỦ</h3>
    <pre>${escapeHtml(output)}</pre>

    <script>
      const textToCopy = ${JSON.stringify(copyText)};
        const textToCopy25 = ${JSON.stringify(copyText25)};
          const textToCopy36 = ${JSON.stringify(copyText36)};
            const textToCopy49 = ${JSON.stringify(copyText49)};
              const textToCopy16 = ${JSON.stringify(copyText16)};
   const textToCopy33 = ${JSON.stringify(copyText33)};
      // Nút 1 (gốc)
      document.getElementById("copyBtn").addEventListener("click", async () => {
        try {
          await navigator.clipboard.writeText(textToCopy);
          alert("✅ Đã copy 50 số vào clipboard!");
        } catch(e) {
      ...
      document.getElementById("copyBtn2").addEventListener("click", async () => {
       await navigator.clipboard.writeText(textToCopy25);
        alert("📄 Copy 2 xong!");
      });

      // Nút 3
      document.getElementById("copyBtn3").addEventListener("click", async () => {
        await navigator.clipboard.writeText(textToCopy36);
        alert("📄 Copy 3 xong!");
      });

      // Nút 4
      document.getElementById("copyBtn4").addEventListener("click", async () => {
        await navigator.clipboard.writeText(textToCopy49);
        alert("📄 Copy 4 xong!");
      });

        // Nút 4
      document.getElementById("copyBtn5").addEventListener("click", async () => {
        await navigator.clipboard.writeText(textToCopy16);
        alert("📄 Copy 5xong!");
      });

            document.getElementById("copyBtn6").addEventListener("click", async () => {
        await navigator.clipboard.writeText(textToCopy33);
        alert("📄 Copy 5xong!");
      });
    </script>
  </body>
